...
- **2026-06-12 (night)** — Game v3 "week-scoped fire" shipped (Fable directed+verified, Sonnet implemented, Opus checker FINDINGS→all fixed). Mechanics: fire attaches to the corrected receipt's WEEK (table `game_week_fires`, migration 0011); 3 flames burn that week only (board burn removed); water cap 5, douse = 1/flame via `POST /game/water/spend {units, week_start_at}`, auto-douse last-resort; streak = DERIVED consecutive clean-green weeks (flames pause it, empty weeks skipped, current week excluded); Skip Pass every 4 green weeks, awarded ONLY from committing paths (post-sync bookkeeping + rebuild — never from GET). Invariant-relevant: gamification stays non-fatal in post-sync bookkeeping (checker-verified); `add_fire` mutations gated on event-record truth (idempotent); rebuild replay respects the debug seed event floor. Frontend: hero week tile + stamps, conditional droplets/Skip Pass chips, "How to play" dialog — ALL game copy now audited against backend mechanics (old copy was inverted). Bug pack same session: `parseApiDate` (naive-UTC fix — killed stuck "1m waiting" + off-UTC shred eligibility), sync-button clipping, Download desktop-only, query-devtools opt-in via localStorage.
- **2026-06-12 (evening)** — Snappy UI simplification pass 2 (commit 6e552d3; Fable directed+implemented, Opus checker APPROVE). Invariant-relevant: quick-sync from the list is now ALWAYS preview-gated (SyncPreviewDialog; skip-preview localStorage flag deliberately cannot bypass it; server remains authoritative on every gate); `toDraftFromPayload` moved verbatim to `src/lib/validation-draft.ts` (shared by detail + list preview); auto-refresh fires pull-only endpoints (`/ingest/scan`, `/ynab/updates/fetch`) on page open / stale refocus — no YNAB write can fire without an explicit user confirm. UI: 2 tabs (To Review + Done; Processing merged into To Review), hamburger removed (search lookup + debug-panel wrench in tab row), debug panel overhauled (maintenance actions w/ descriptions, "Your name" greeting field, seed under Advanced), Snappy greetings + 35 source-verified quotes (`snappy-quotes.ts` — never add quotes from memory; verify against primary sources), confetti shredded icon, dev indicators gated on `debug_tools_enabled` (now exposed on GET /api/config).
- **2026-06-12** — UX/automation batch complete (4 phases, commits 98e1aaf, 66dafea, 784a6e5, 5159b10; Fable directed, Sonnet implemented, Opus planned Phase 4 + checked Phases 1 and 4, both APPROVE). New invariant-relevant facts: validation payloads carry provenance fields `account_source`/`category_source` which are STRIPPED from payload-change equality (`payloads_equivalent`); payee→category memory (table `payee_category_memory`, migration 0009) learns on CREATED syncs only (never MATCHED_UPDATED adopt path) and can only ever write category ids — amounts always flow through `recompute_payload_from_workspace`; list `sync_ready` gates mirror `/sync` and the server remains authoritative; skip-preview requires zero strip reasons, lock warnings, and ambiguity flags. Live E2E: quick-sync created YNAB txn 2fba733a in test budget, split template learned (trader joes, 29 items).
